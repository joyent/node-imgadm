 ...
 * @param callback {Function} `function (err, imagesInfo)`
 */
IMGADM.prototype._loadLxcImages = function _loadLxcImages(zpool, callback) {
    var self = this;
    ...
    execPlus({
        ...
        async.forEachSeries(lxcImages, function loadOne(lxcImage, next) {
            ...
            var opts = {uuid: uuid, zpool: zpool};
            self._db.loadImage(opts, function (err, info) {
                if (err) {
                    next(err);
                    return;
 ...
 *
 * @param opts {Object}
 *          - sysinfo {Object} Result from a sysinfo call.
 * @param callback {Function} `function (err, imagesInfo)`
 */
IMGADM.prototype._loadImages = function _loadImages(opts, callback) {
    ...
    if (typeof (opts) === 'function') {
        ...
        getSysinfo(function (err, sysinfo) {
            ...
            self._loadImages({sysinfo: sysinfo}, callback);
        });
    ...
    assert.object(opts, 'opts');
    assert.object(opts.sysinfo, 'opts.sysinfo');
    assert.string(opts.sysinfo.Zpool, 'opts.sysinfo.Zpool');

    var zpool = opts.sysinfo.Zpool;

    this._loadLxcImages(zpool, callback);
};
