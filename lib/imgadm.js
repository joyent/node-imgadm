...
var JOYENT_BRANDS = ['joyent', 'joyent-minimal'];

// Various commands.  Linux is indecisive about /bin vs. /usr/bin.  SmartOS has
// a symbolic link from /bin to /usr/bin.
var BZIP2 = '/bin/bzip2';
var CHROOT_GTAR = '/usr/img/sbin/chroot-gtar';
var GZIP = '/bin/gzip';
var LXC = '/usr/local/lxd/bin/lxc';
var MKDIR = '/bin/mkdir';
var SYSINFO = '/usr/bin/sysinfo';
var XZ = '/usr/bin/xz';
var ZFS = '/sbin/zfs';
...
// XXX-mg sdc-cn-agent/lib/backends/linux/sysinfo.js needs to be its own module.
// and used here.
var fake_sysinfo = {
    'Live Image': '20190314T232247Z',
    'System Type': 'Linux',
    'Boot Time': '1553699146',
    'SDC Version': '7.0',
    'Manufacturer': 'VMware, Inc.',
    'Product': 'VMware Virtual Platform',
    'Serial Number': 'VMware-56 4d 29 35 8e 55 a5 86-bd 5c 5b 18 1e ab ce fc',
    'SKU Number': '',
    'HW Version': 'None',
    'HW Family': '',
    'Setup': 'false',
    ...
    'Admin NIC Tag': 'admin',
    'Admin IP': 'dhcp',
    'UUID': '564d2935-8e55-a586-bd5c-5b181eabcefc',
    'Hostname': 'mock',
    'CPU Total Cores': 4,
    'MiB of Memory': '8191',
    'Zpool': 'triton',
    ...
    'Zpool Size in GiB': 32,
    'Disks': {
        'c2t2d0': {'Size in GB': 80}
    },
    'Boot Parameters': {
        'console': 'ttya',
        'root_shadow':
    ...
    'Network Interfaces': {
        'e1000g0': {
            'MAC Address': '00:0c:29:ab:ce:fc',
            'ip4addr': '10.88.88.128',
            'Link Status': 'up',
            'NIC Names': ['external', 'new_tag2', 'admin', 'new_tag1']
        }
    },
...
function getSysinfo(callback) {
    ...
    // XXX-mg hack around missing sysinfo
    callback(null, JSON.parse(JSON.stringify(fake_sysinfo)));

    // JSSTYLED
    /* XXX-mg
    execFile(SYSINFO, function (err, stdout, stderr) {
        if (err) {
            callback(err);
        } else {
...
function zfsDestroy(dataset, log, callback) {
    ...
    assert.func(callback, 'callback');
    var cmd = format('%s destroy -r %s', ZFS, dataset);
...
function zfsRenameSnapshot(a, b, options, callback) {
    ...
    var cmd = format('%s rename%s %s', ZFS, optStr, a, b);
...
function getZfsDataset(name, properties, callback) {
    ...
    function getDataset(next) {
        var cmd = format('%s list -H -p -o %s %s',
            ZFS, properties.join(','), name);
    ...
    function getChildSnapshots(next) {
        ...
        dataset.children = {};
        var cmd = format('%s list -t all -pHr -o name %s', ZFS, name);
        exec(cmd, {maxBuffer: 10485760}, function (err, stdout, stderr) {
            if (err) {
                next(new errors.InternalError({
                    cause: err,
    ...
    function getDependentClones(next) {
        ...
        var cmd = ZFS + ' list -t filesystem,volume -o origin,name -pH';
        exec(cmd, {maxBuffer: 10485760}, function (err, stdout, stderr) {
            if (err) {
                next(new errors.InternalError({
                    cause: err,
...
/*
 * Load ZFS images from the system and merge in manifest data from the imgadm
 * cache/database.
 *
 * @param callback {Function} `function (err, imagesInfo)`
 */
IMGADM.prototype._loadZfsImages = function _loadZfsImages(callback) {
    var self = this;
    var i;
    ...
    // "$zpoolname/$uuid" whose mountpoint is not a zone root, or a lxc image
    // (for Linux). Full images won't have an origin, incremental images will.
    //
    ...
    /* END JSSTYLED */
    //
    // XXX-mg this previously listed all the zones to figure out which datasets
    // belonged to zones and which were for images.  We obviously don't have
    // zones so we need to tap into another interface to figure out which
    // datasets are images vs. instances.
    execPlus({
        command: '/bin/true',
        log: self.log,
         ...
         */
        execPlus({
            command: ZFS + ' list -t filesystem,volume,snapshot -pH '
...
/*
 * Load LXC images from the system and merge in manifest data from the imgadm
 * cache/database.
 *
 * @param opts {Object}
 *          - uuid {String} Optional image id to load.
 *          - zpool {String}
 *
 * @param callback {Function} `function (err, imagesInfo)`
 */
IMGADM.prototype._loadLxcImages = function _loadLxcImages(opts, callback) {
    assert.object(opts, 'opts');
    ...
    var self = this;
    var cmd = LXC + ' image list --format json';

    if (opts.uuid) {
        cmd += ' ' + common.lxdFingerprintFromUuid(opts.uuid);
    }

    execPlus({
        command: cmd,
        log: self.log,
        errMsg: 'could not run "lxc image list"',
        execOpts: {
            maxBuffer: 10485760  /* >200k hit in prod, 10M should suffice */
        }
    }, function (lxcErr, stdout, stderr) {
        if (stderr) {
            self.log.info({err: lxcErr}, 'lxc image list stderr: %s', stderr);
        }
        ...
        var lxcImages = JSON.parse(stdout);
        var imagesInfo = [];

        async.forEachSeries(lxcImages, function loadOne(lxcImage, next) {
            var uuid = common.uuidFromSha256(lxcImage.fingerprint);
            var loadOpts = {uuid: uuid, zpool: opts.zpool};
            self._db.loadImage(loadOpts, function (err, info) {
                if (err) {
                    next(err);
                    return;
                }
 ...
 *
 * @param opts {Object}
 *          - uuid {Object} Result from a sysinfo call.
 *          - zpool {String} zpool to use.
 * @param callback {Function} `function (err, imagesInfo)`
 */
IMGADM.prototype._loadImages = function _loadImages(opts, callback) {
    var self = this;
    ...
    if (typeof (opts) === 'function') {
        ...
        getSysinfo(function (err, sysinfo) {
            ...
            self._loadImages({zpool: sysinfo.Zpool}, callback);
        });
    ...
    this._loadLxcImages(opts, callback);
};
...
IMGADM.prototype.getImage = function getImage(options, callback) {
    ...
    assert.optionalString(options.zpool, 'options.zpool');
    ...
    var imageInfo = null;
    vasync.waterfall([
        function gatherSysinfo(next) {
            if (options.zpool !== undefined) {
                next(null);
                return;
            }
            getSysinfo(function (err, sysinfo) {
                ...
                options.zpool = sysinfo.Zpool;
                next();
            });
        },
        function doGetZfsDataset(next) {
            // We arn't using zfs datasets right now.
            if (true || 1) {
                next();
                return;
            }
            ...
            getZfsDataset(name, properties,
                function gotDataset(zfsErr, dataset) {
        ...
        function doGetLxcImage(next) {
            self._loadImages(options, function (err, imagesInfo) {
                if (err) {
                    next(err);
                    return;
                }
                ...
                if (imagesInfo.length) {
                    imageInfo = imagesInfo[0];
                }
                next();
            });
        }],
        function getSeriesDone(err) {
            if (err) {
                callback(err, null);
                return;
            }
...
IMGADM.prototype.deleteImage = function deleteImage(options, callback) {
    ...
    vasync.pipeline({funcs: [
        ...
        function checks(_, next) {
            ...
            self.getImage(getOpts, function (err, imageInfo) {
                if (err) {
                    ...
                    imageType = imageInfo.manifest.type;
                    next();
                }
            });
        },

        function delZfs(_, next) {
            if (imageType === 'lxd') {
                next();
                return;
            }
            ...
            execPlus({
                command: format('%s destroy -r %s/%s', ZFS, zpool, uuid),
                log: self.log,
        ...
        function delLxd(_, next) {
            if (imageType !== 'lxd') {
                next();
                return;
            }

            var cmd = format('%s image delete %s',
                LXC, common.lxdFingerprintFromUuid(uuid));
            execPlus({
                command: cmd,
                log: self.log,
                errMsg: format('error deleting image "%s"', uuid)
 ...
 *      }
 */
IMGADM.prototype._downloadImageFile =
function _downloadImageFile(fileObj, fileIndex, ee, opts, cb) {
    var self = this;

    // As from `manifest.files[x]`.
    assert.object(fileObj, 'fileObj');
    assert.optionalNumber(fileObj.size, 'fileObj.size');
    assert.optionalString(fileObj.sha1, 'fileObj.sha1');
    assert.object(opts, 'opts');
    ...
    var downFile = common.downloadFileFromUuid(uuid, fileIndex);
    var context = {};
    ...
    vasync.pipeline({arg: context, funcs: [
        function skipIfPreDownloaded(ctx, next) {
            ...
            if (fileObj.sha1) {
                var checkOpts = {
                    file: downFile,
                    checksum: 'sha1:' + fileObj.sha1
                };
                checkFileChecksum(checkOpts, function (err) {
                    if (err) {
                        log.info({err: err, uuid: uuid, downFile: downFile,
                            expectedSha1: fileObj.sha1},
                            'unexpected sha1 checksum for pre-downloaded '
        ...
        function getStream(ctx, next) {
            opts.importInfo.index = fileIndex;
        ...
        function downloadIt(ctx, next_) {
            ...
            if (!zstream && fileObj.sha1) {
                ctx.checksum = ['sha1', fileObj.sha1];
        ...
        if (err) {
            ...
            var downloadInfo = {
                path: downFile,
                size: context.bytesDownloaded || fileObj.size
            };
...
/*
 * Download all image files to a local temp directory.
 *
 * @param opts {Object}
 *      - source
 *      - bar
 ...
 *        the GetImageFile will be a raw ZFS dataset stream.
 * @param cb {Function} `function (err, downloadInfo)` where `downloadInfo` is
 *        an array of info objects (usually just one):
 *   [
 *      {
 *          "path": "/var/tmp/.imgadm-downloads/$uuid.file",
 *          "size": <content-length>
 ...
 *   ]
 */
IMGADM.prototype._downloadAllImageFiles =
function _downloadAllImageFiles(opts, cb) {
    assert.object(opts, 'opts');
    assert.object(opts.imgMeta, 'opts.imgMeta');
    assert.object(opts.imgMeta.manifest, 'opts.imgMeta.manifest');
    assert.arrayOfObject(opts.imgMeta.manifest.files,
        'opts.imgMeta.manifest.files');

    var self = this;
    var manifest = opts.imgMeta.manifest;
    ...
    vasync.forEachPipeline({inputs: files,
        func: function _eachFile(fileObj, next) {
            fileIndex += 1;
            self._downloadImageFile(fileObj, fileIndex, ee, opts,
                    function _onDownloadImageFileCb(err, dlInfo) {
                if (err) {
                    next(err);
                    return;
                }
        ...
        }}, function _onDownloadAllFilesPipelineCb(err) {
            cb(err, dlInfos);
        });
...
IMGADM.prototype._removeImageDownloads =
function _removeImageDownloads(irec, cb) {
    assert.object(irec, 'irec');
    assert.arrayOfObject(irec.dlInfoArray, 'irec.dlInfoArray');

    vasync.forEachParallel({inputs: irec.dlInfoArray,
        func: function _removeDownloadFile(dlInfo, next) {
            rimraf(dlInfo.path, next);
        }
    }, cb);
};
        ...
        function createIrecs(ctx, next) {
            vasync.parallel({funcs: [
                function loadInstalledImages(nextGather) {
                    TIME('loadImages');
                    self._loadImages({zpool: zpool},
                            function onLoadImages(err, imagesInfo) {
                        TIME('loadImages');
                        if (err) {
                            nextGather(err);
                            return;
        ...
        function downloadAndInstall(ctx, next) {
            ...
            var downloadQ = vasync.queuev({
                ...
                worker: function fetchImg(irec, nextDownload) {
                    ...
                    var dlEvents = self._downloadAllImageFiles(dlOpts,
                            function (err, dlInfoArray) {
                        if (err) {
                            nextDownload(err);
                            return;
                        }

                        var totalSize = dlInfoArray.reduce(
                            function _sumSize(currentValue, info) {
                                return (currentValue + info.size);
                            }, 0);
                        self.log.info(
                            {uuid: irec.uuid, downloadInfo: dlInfoArray},
                            'downloaded image');
                        barLogCb(format('Downloaded image %s (%s)',
                            irec.uuid, common.humanSizeFromBytes(totalSize)));
                        irec.dlInfoArray = dlInfoArray;
            ...
            var installQ = vasync.queuev({
                ...
                worker: function installImg(irec, nextInstall) {
                    ...
                    var installOpts = {
                        ...
                        filePath: irec.dlInfoArray[0].path,
                        zstream: zstream,
                        quiet: opts.quiet,
                        logCb: barLogCb
                    };
                    self._installSingleImage(installOpts, function (err) {
                        ...
                        self._removeImageDownloads(irec, function (rmErr) {
                            if (rmErr) {
                                nextInstall(rmErr);
                                return;
                            }
                            ...
                            Object.keys(onDeck).forEach(function (uuid) {
                                ...
                                if (origin === irec.uuid) {
                                    log.debug({uuid: onDeckIrec.uuid},
            ...
            var haveSizes = (!zstream &&
                irecs.filter(function (irec) {
                    return (irec.imgMeta.manifest.files.filter(function (f) {
                        return (f.hasOwnProperty('size'));
                    })).length === irec.imgMeta.manifest.files.length;
                }).length === irecs.length);
            var barOpts = {
            ...
            if (haveSizes) {
                var totalBytes = irecs.reduce(function (irecValue, irec) {
                    return (irecValue + irec.imgMeta.manifest.files
                        .reduce(function (currentValue, f) {
                            return (currentValue + f.size);
                        }, 0));
                    }, 0);
...
IMGADM.prototype._installDockerImage = function _installDockerImage(ctx, cb) {
    ...
    assert.optionalString(ctx.zpool, 'ctx.zpool');
    ...
    vasync.pipeline({funcs: [
        ...
        function zfsSnapshot(_, next) {
            var argv = [ZFS, 'snapshot', partialDsName + '@final'];
            execFilePlus({argv: argv, log: log}, next);
        },
        ...
        function renameToFinalDsName(_, next) {
            var argv = [ZFS, 'rename', partialDsName, ctx.dsName];
            execFilePlus({argv: argv, log: log}, next);
        }
...
/*
 * This handles creating an image in the lxd images from a lxd image.
 *
 * Dev Note: This presumes an imgadm lock is held for this image.
 */
IMGADM.prototype._installLxdImage = function _installLxdImage(ctx, cb) {
    assert.object(ctx, 'ctx');
    assert.object(ctx.irec, 'ctx.irec');
    assert.arrayOfObject(ctx.irec.dlInfoArray, 'ctx.irec.dlInfoArray');
    assert.string(ctx.filePath, 'ctx.filePath');
    assert.func(cb, 'cb');
    ...
    if (ctx.irec.dlInfoArray.length == 1) {
        // A combined (one file) lxd image.
        argv = [LXC, 'image', 'import', ctx.filePath];
    } else {
        // A split (two files) lxd image.
        assert.equal(ctx.irec.dlInfoArray.length, 2,
            'ctx.irec.dlInfoArray.length === 2');
        var rootFilePath = ctx.irec.dlInfoArray[1].path;
        argv = [LXC, 'image', 'import', ctx.filePath, rootFilePath];
    }

    execFilePlus({argv: argv, log: log}, function (err, stdout, stderr) {
        if (err) {
            log.warn({argv: argv, err: err, stdout: stdout, stderr: stderr},
                'error installing lxd image');
        }
        cb(err);
    });
};
    ...
    vasync.pipeline({funcs: [
        ...
        function recvTheDataset(_, next) {
            ...
            if (compression === 'bzip2') {
                uncompressor = spawn(BZIP2, ['-cdfq']);
                numToFinish++;
            } else if (compression === 'gzip') {
                uncompressor = spawn(GZIP, ['-cdfq']);
                numToFinish++;
            } else if (compression === 'xz') {
                uncompressor = spawn(XZ, ['-cdfq']);
                numToFinish++;
            } else {
            ...
            var zfsRecv = spawn(ZFS, ['receive', ctx.partialDsName]);
         ...
         */
        function renameToFinalDsName(_, next) {
            var cmd = format('%s rename %s %s',
                ZFS, ctx.partialDsName, ctx.dsName);
        ...
        vasync.pipeline({funcs: [
            ...
            function rollbackPartialDsIfNecessary(_, next) {
                if (err && ctx.partialDsName) {
                    ...
                    // dataset was created.
                    var cmd = format('%s destroy -r %s',
                        ZFS, ctx.partialDsName);
...
IMGADM.prototype._installSingleImage = function _installSingleImage(ctx, cb) {
    ...
    vasync.pipeline({funcs: [
        ...
        function _installTheFile(_, next) {
            ...
            } else if (!zstream && manifest.type === 'lxd') {
                self._installLxdImage(ctx, next);
            } else {
        ...
        if (err && ctx.installedDs) {
            var cmd = format('%s destroy -r %s', ZFS, ctx.dsName);
            exec(cmd, function (rollbackErr, stdout, stderr) {
                if (rollbackErr) {
...
IMGADM.prototype.createImage = function createImage(options, callback) {
    ...
    async.waterfall([
        ...
        function validateMaxOriginDepth(next) {
            ...
            async.whilst(
                function () {
                    return (currentDepth <= maxOriginDepth &&
                        !foundFirstOrigin);
                },
                function (cb) {
        ...
        function gatherManifest(next) {
            ...
            if (JOYENT_BRANDS.indexOf(vmInfo.brand) !== -1 &&
        ...
        function sendImageFile(next) {
            ...
            if (compression === 'none') {
                ...
                compressor = spawn(BZIP2, ['-cfq']);
                imageInfo.filePath += '.bz2';
                numToFinish++;
            } else if (compression === 'gzip') {
                compressor = spawn(GZIP, ['-cfq']);
                imageInfo.filePath += '.gz';
                numToFinish++;
            } else if (compression === 'xz') {
                compressor = spawn(XZ, ['-cfq']);
            ...
            self.log.debug({cmd: [ZFS].concat(zfsArgs)},
                'spawn zfs send');
            var zfsSend = spawn(ZFS, zfsArgs);
            var zfsStderrChunks = [];
