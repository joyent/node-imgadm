        ...
        if (manifest) {
            if (opts.ensureActive) {
                try {
                    ...
                    cb(new errors.InvalidManifestError(err2));
                    return;
                }
            }
...
ImgapiSource.prototype.getImgFileStream = function getImgFileStream(opts, cb) {
    assert.object(opts, 'opts');
    assert.string(opts.uuid, 'opts.uuid');
    assert.optionalNumber(opts.index, 'opts.index');
    assert.func(cb, 'cb');

    var account = null;
    var options = {index: opts.index};

    this.client.getImageFileStream(opts.uuid, account, options, cb);
};
