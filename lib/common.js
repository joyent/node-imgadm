...
function downloadFileFromUuid(uuid, index) {
    assert.string(uuid, 'uuid');
    assert.optionalNumber(index, 'index');
    var downloadPath = path.join(DOWNLOAD_DIR, uuid + '.file');
    if (index) {
        downloadPath += index;
    }
    return (downloadPath);
...
// Return a lxd fingerprint (sha256) prefix.
function lxdFingerprintFromUuid(uuid) {
...
module.exports = {
    ...
    execPlus: execPlus,
    lxdFingerprintFromUuid: lxdFingerprintFromUuid,
    uuidFromSha256: uuidFromSha256,
